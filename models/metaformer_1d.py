...
from timm.layers import trunc_normal_, DropPath, GroupNorm1, LayerNorm, use_fused_attn
from timm.models import build_model_with_cfg, register_model
from timm.models.layers import is_fast_norm, fast_layer_norm
...
class FastAdaptiveAvgPool(nn.Module):
    ...
    def forward(self, x):
        return x.mean(dim=(-1,), keepdim=not self.flatten)


class Stem(nn.Module):
    """
    Stem implemented by a layer of convolution.
    Conv1d params constant across all models.
    """
...
class Scale(nn.Module):
    ...
    def __init__(self, dim, init_value=1.0, trainable=True, use_nchw=True):
        super().__init__()
        self.shape = (dim, 1) if use_nchw else (dim,)
...
class LayerNorm1d(nn.LayerNorm):
    """ LayerNorm for channels of '1D' spatial NCL tensors """
...
class LayerNorm1dNoBias(LayerNorm1d):
    def __init__(self, num_channels, **kwargs):
        super().__init__(num_channels, **kwargs)
        self.eps = kwargs.get('eps', 1e-6)
        self.bias = None
...
class SepConv(nn.Module):
    ...
    def __init__(
        ...
        self.pwconv1 = nn.Conv1d(dim, mid_channels, kernel_size=1, bias=bias)
        ...
        self.pwconv2 = nn.Conv1d(mid_channels, dim, kernel_size=1, bias=bias)
...
class MetaFormer(nn.Module):
    ...
    def __init__(
            ...
            downsample_norm=LayerNorm1dNoBias,
            norm_layers=LayerNorm1dNoBias,
        ...
        self.head = nn.Sequential(OrderedDict([
            ('global_pool', FastAdaptiveAvgPool()),
    ...
    def reset_classifier(self, num_classes=0, global_pool=None):
        if global_pool is not None:
            self.head.global_pool = FastAdaptiveAvgPool()
...
def caformer_s18(pretrained=False, **kwargs) -> MetaFormer:
    model_kwargs = dict(
        ...
        norm_layers=[LayerNorm1dNoBias] * 2 + [LayerNormNoBias] * 2,
        **kwargs)
...
if __name__ == "__main__":
    ...
    # using pre-defined model (pooling as token-mixer)
    model = poolformer_s12(in_chans=in_chans, num_classes=num_classes)
    print(x.shape, model(x).shape)

    # using pre-defined model (CNN as token-mixer)
    model = convformer_s18(in_chans=in_chans, num_classes=num_classes)
    print(x.shape, model(x).shape)

    # using pre-defined model (CNN+Attention as token-mixer)
    model = caformer_s18(in_chans=in_chans, num_classes=num_classes)
    print(x.shape, model(x).shape)

    # using custom defined model: 3 stages CNN
    model_args = dict(in_chans=in_chans, depths=[2, 6, 4], dims=[64, 128, 256],
                      token_mixers=SepConv, norm_layers=LayerNorm1dNoBias, num_classes=num_classes)
    model = _create_metaformer('convformer_custom', pretrained=False, **model_args)
    print(x.shape, model(x).shape)

...
from timm.layers import DropPath, LayerType, pad_same, create_attn, \
    get_attn, get_act_layer, get_norm_layer
...
class AvgPool1dSame(nn.AvgPool1d):
    ...
    def __init__(self, kernel_size: int, stride=None, padding=0, ceil_mode=False, count_include_pad=True):
        super(AvgPool1dSame, self).__init__(kernel_size, stride, padding, ceil_mode, count_include_pad)
...
def downsample_avg(
    ...
    if stride == 1 and dilation == 1:
        ...
        avg_pool_fn = AvgPool1dSame if avg_stride == 1 and dilation > 1 else nn.AvgPool1d
...
class ResNet(nn.Module):
    ...
    def __init__(
            ...
            norm_layer: LayerType = nn.BatchNorm1d,
            aa_layer: Optional[Type[nn.Module]] = None,
            drop_rate: float = 0.0,
            drop_path_rate: float = 0.,
            drop_block_rate: float = 0.,
        ...
        if deep_stem:
            ...
            self.conv1 = nn.Sequential(*[
                nn.Conv1d(in_chans, stem_chs[0], 3, stride=2, padding=1, bias=False),
                norm_layer(stem_chs[0]),
                act_layer(inplace=True),
                nn.Conv1d(stem_chs[0], stem_chs[1], 3, stride=1, padding=1, bias=False),
                norm_layer(stem_chs[1]),
                act_layer(inplace=True),
                nn.Conv1d(stem_chs[1], inplanes, 3, stride=1, padding=1, bias=False)])
        else:
            self.conv1 = nn.Conv1d(in_chans, inplanes, kernel_size=7, stride=2, padding=3, bias=False)
        ...
        if replace_stem_pool:
            self.maxpool = nn.Sequential(*filter(None, [
                nn.Conv1d(inplanes, inplanes, 3, stride=1 if aa_layer else 2, padding=1, bias=False),
        ...
        # Feature Blocks
        self.channels = channels  # default to [64, 128, 256, 512]
        ...
        final = nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def get_classifier(self, name_only: bool = False):
        return 'fc' if name_only else self.head.fc
...
if __name__ == "__main__":
    ...
    model = resnet18(in_chans=in_chans, num_classes=num_classes)
    print(x.shape, model(x).shape)

    # using pre-defined model
    model = resnet50(in_chans=in_chans, num_classes=num_classes)
    print(x.shape, model(x).shape)

...
class ResNestBottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.avd_first = nn.AvgPool1d(3, avd_stride, padding=1) if avd_stride > 0 and avd_first else None
        ...
        self.avd_last = nn.AvgPool1d(3, avd_stride, padding=1) if avd_stride > 0 and not avd_first else None
...
if __name__ == "__main__":
    ...
    model = resnest14d(in_chans=in_chans, num_classes=num_classes)
    print(x.shape, model(x).shape)

    # using pre-defined model
    model = resnest50d(in_chans=in_chans, num_classes=num_classes)
    print(x.shape, model(x).shape)

...
class Bottle2neck(nn.Module):
    ...
    def __init__(
        ...
        self.conv3 = nn.Conv1d(width * scale, outplanes, kernel_size=1, bias=False)
...
if __name__ == "__main__":
    ...
    model = res2net50_26w_4s(in_chans=in_chans, num_classes=num_classes)
    print(x.shape, model(x).shape)

    # using pre-defined model
    model = res2next50(in_chans=in_chans, num_classes=num_classes)
    print(x.shape, model(x).shape)
